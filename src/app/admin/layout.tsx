  ...
  return (
    <div className="min-h-dvh bg-gray-100 dark:bg-gray-900 flex" data-admin-root>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* Mobile FAB + Popup menu */}
        <div className="md:hidden fixed bottom-6 right-6 z-50">
          {menuOpen && (
            <>
              <div className="fixed inset-0" onClick={() => setMenuOpen(false)} />
              <div className="absolute bottom-16 right-0 bg-white dark:bg-gray-800 rounded-xl shadow-xl border dark:border-gray-700 py-2 w-48 animate-slide-in">
                {sidebarLinks.filter(l => !l.adminOnly || role === "superadmin").map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    onClick={() => setMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 text-sm text-gray-600 dark:text-gray-300 hover:bg-gold-50 dark:hover:bg-gold-900/30 hover:text-gold-700 dark:hover:text-gold-400 transition-colors"
                  >
                    <link.icon className="w-5 h-5" />
                    <span>{t(link.nameKey)}</span>
                  </Link>
                ))}
                <hr className="my-1 mx-2 dark:border-gray-700" />
                <Link
                  href="/"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-3 px-4 py-3 text-sm text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                >
                  <HiOutlineArrowLeft className="w-5 h-5" />
          ...
          <button
            ...
            className="w-14 h-14 rounded-full bg-gold-600 text-white shadow-lg hover:bg-gold-700 transition-colors flex items-center justify-center"
          >
